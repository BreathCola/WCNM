#!/usr/bin/env python3
"""Plan or execute the D-016 internal-object T-color recovery pilot."""
...
TO_20000_OUTPUT = (
    ROOT / "output/stage_d_tihubird_c03r8_internal_object_townership_15500_20000_v1"
)
TO_20000_AUDIT = TO_20000_OUTPUT / "internal_object_townership_to_20000_cpu_audit.json"
PILOT_OUTPUT = ROOT / "output/stage_d_tihubird_c03r8_internal_object_townership_pilot_v1"
PILOT_SOURCE = PILOT_OUTPUT / "chkpnt15500.pth"
...
def _validate_git_for_execute() -> None:
    ...
    if identity["ahead_behind"] not in ("0\t0", "0 0"):
        raise RuntimeError(f"local/remote parity is not 0/0: {identity['ahead_behind']}")
...
def _validate_source_checkpoint(path: Path) -> dict:
    if sha256_file(path) != INTERNAL_OBJECT_COLOR_RECOVERY_SOURCE_SHA256:
    ...
    optimizer = transmittance.get("optimizer")
    if optimizer is None:
        raise RuntimeError("BLOCKED_BY_PARAMETER_GROUP_CONTRACT: missing T optimizer")
    group_names = [group.get("name") for group in optimizer.get("param_groups", [])]
    expected = list(INTERNAL_OBJECT_COLOR_RECOVERY_TRAINABLE_T_GROUPS) + list(
        INTERNAL_OBJECT_COLOR_RECOVERY_FROZEN_T_GROUPS
    )
    if sorted(group_names) != sorted(expected):
        raise RuntimeError(
            "BLOCKED_BY_PARAMETER_GROUP_CONTRACT: T optimizer groups are not separable"
        )
    if "color" not in group_names or "opacity" not in group_names:
        raise RuntimeError(
            "BLOCKED_BY_PARAMETER_GROUP_CONTRACT: T appearance/opacity split is missing"
        )
    ...
    prior_cache = config.get("cached_t_warmup", {})
    pilot = torch.load(PILOT_SOURCE, map_location="cpu")
    if state_sha256(checkpoint["diffuse"]) != state_sha256(pilot["diffuse"]):
        raise RuntimeError("source D identity does not match pilot 15500 D")
    if state_sha256(checkpoint["reflection"]) != state_sha256(pilot["reflection"]):
        raise RuntimeError("source R identity does not match pilot 15500 R")
    ...
    prior = config.get("internal_object_to_20000", {})
    if prior.get("schema") != "rtgs_stage_d_internal_object_townership_to_20000_v1":
        raise RuntimeError("source checkpoint is not from the D-016 to-20000 continuation")
    return checkpoint
...
def _validate_to_20000_audit(path: Path) -> dict:
    ...
    if recommended.get("best_object_ownership") != 20000:
        raise RuntimeError("to-20000 CPU audit best_object_ownership reference mismatch")
    return audit
...
def training_command(args: argparse.Namespace) -> list[str]:
    return [
        ...
        "--model_type", "surfel",
        "--stage", "stage_d",
        "--experiment", "TiHuBird C03-r8 Stage D D-016 internal-object T color recovery",
        "--resolution", "8",
        ...
        "--ray_chunk_size", "2048",
        "--iterations", str(INTERNAL_OBJECT_COLOR_RECOVERY_ENDPOINT),
        "--start_checkpoint", str(SOURCE),
        "--lambda_spec", "0.2",
        "--specular_k0", "0.9",
        "--lambda_depth", "0.2",
        "--stage_d_depth_start_iteration", "40000",
        "--stage_d_internal_object_color_recovery",
        "--stage_d_phase_a_end_iteration", str(INTERNAL_OBJECT_COLOR_RECOVERY_ENDPOINT),
        ...
        "--disable_viewer",
        "--quiet",
        "--save_iterations", *[str(node) for node in INTERNAL_OBJECT_COLOR_RECOVERY_NODES],
        "--checkpoint_iterations", *[str(node) for node in INTERNAL_OBJECT_COLOR_RECOVERY_NODES],
    ]


def build_plan(args: argparse.Namespace) -> dict:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing existing D-016 color-recovery output: {OUTPUT}")
    ...
    audit = _validate_to_20000_audit(TO_20000_AUDIT)
    ...
    optimizer_groups = source_checkpoint["transmittance"]["optimizer"]["param_groups"]
    return {
        "schema": PLAN_SCHEMA,
        "execute": bool(args.execute),
        "output": str(OUTPUT),
        "source": str(SOURCE),
        "source_sha256": INTERNAL_OBJECT_COLOR_RECOVERY_SOURCE_SHA256,
        "source_selection_reason": {
            "best_rgb": audit["recommended_review_candidates"]["best_rgb"],
            "best_leakage_tradeoff": audit["recommended_review_candidates"]["best_leakage_tradeoff"],
            "best_object_ownership_reference": audit["recommended_review_candidates"]["best_object_ownership"],
        ...
        "to_20000_audit": {
            "path": str(TO_20000_AUDIT),
            "verdict": audit.get("verdict"),
            "technical_run_complete": audit.get("technical_run_complete"),
            "errors": audit.get("errors"),
            "recommended_review_candidates": audit.get("recommended_review_candidates"),
        },
        "release": str(RELEASE),
        ...
        "iterations": {
            "start": 16500,
            "first_update": 16501,
            "end_inclusive": INTERNAL_OBJECT_COLOR_RECOVERY_ENDPOINT,
            "updates": 500,
            "transmittance_local": [1501, 2000],
        },
        "nodes": list(INTERNAL_OBJECT_COLOR_RECOVERY_NODES),
        "transmittance": {
            "count": 4096,
        ...
        "parameter_group_contract": {
            ...
            "source_optimizer_group_lrs": {
                group["name"]: float(group.get("lr", 0.0)) for group in optimizer_groups
            },
            "opacity_lr_multiplier": 0.0,
            "blocked_if_not_separable": "BLOCKED_BY_PARAMETER_GROUP_CONTRACT",
        },
        "frozen_branches": {
            "diffuse": True,
            "reflection": True,
            "transmittance_geometry_opacity": True,
            "transmittance_color": False,
        },
        "losses": {
            ...
            "cin_target": (
                "inside_contribution target = clamp(gt_rgb - detach(final_t_off) "
                "- detach(cout_contribution), 0, 1)"
            ),
        ...
        "renderer": {
            "transparent_direct_mode": "off",
            "transparent_reflection_mode": "off",
            "dr_transparent_region_participation": False,
            "ray_domain_changed": False,
        },
        "git": _git_identity(),
        "command": command,
        "note": "Default mode is plan-only; --execute launches the D-016 T-color recovery pilot.",
    }
...
def main() -> int:
    ...
    parser.add_argument("--lambda-object-cin-color", type=float, default=0.10)
